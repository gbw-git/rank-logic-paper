...
% \begin{lem}
%   Let $g$ be a matrix symmetric gate in $C_n$ with children $H$. Let $\alpha \in
%   U^{\underbar{\consp(g)}}$ and $\gamma: U \rightarrow [n]$ such that
...
%   \in U^{\underbar{\consp(g)}}$ and let $\gamma: U \rightarrow [n]$ such that
...
\begin{proof}
  proof in book
  % So $\row (\sigma h_1) = \row (h_2)$, and so $\sigma \vec{r} = \vec{r}_{\sigma h_1} =
  % \vec{r}_{h_2}$.
\end{proof}
