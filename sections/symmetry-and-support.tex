...
% \begin{definition}
%   Let $G \leq X$ be a set on which a left group action of $G$ on $X$ is
%   defined and let $x \in X$. Then $\stab_G (x) = \{\pi \in G : \pi x = x\}$
%   and $\orb_G (x) = \{\pi x : \pi \in G\}$. In the event that the context
%   makes the group obvious we omit the subscript. We use $\stab_n$ and $\orb_n$
%   to abbreviate $\stab_{\sym_n}$ and $\orb_{\sym_n}$. Let $Y$ be a set on
%   which a left group action of $G$ on $Y$ is defined and let $y \in Y$. Then
%   we let $\stab_y(x) := \stab_{\stab_G(y)}(x)$ and $\orb_y(x) :=
%   \orb_{\stab_{G}(y)}(x)$.
% \end{definition}
...
% Let $X$ is a set on which a left group action of $G \leq \sym_n$ is defined,
% and let $x \in X$. We often let the group $G$ be the stabiliser group of some
% other set (or element) $S$ on which the action of $\sym_n$ is defined. In this
% case we write $\SP_S(x)$ and $\consp_S(x)$ to abbreviate $\SP_{\stab(S)}(x)$
% and $\consp_{\stab(S)}(x)$.
...
% GW: Is this needed? It might have small support already? I want to clean this
% up.
