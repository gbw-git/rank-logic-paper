...
% AD: This is a more informal explanation to replace that below. In any case,
% the formal definition follows.

% must be larger than the entire circuit. In particular, we prove that if a set
% $S \subseteq \sym_n \times H^2_g$ is \emph{useful} and \emph{independent} then
% the size of the powerset of $S$ is a lower bound on the size of the orbit
% (i.e.\ $\vert \orb(g) \vert = [\sym_n : \stab_n(g)] \geq 2^S$). We then show
% that if a gate $g$ has a large canonical support, then we can can construct a
% set $S$ that satisfies these two conditions and such that $2^S$ exceeds the
% size of circuit, hence proving the result by contradiction.
