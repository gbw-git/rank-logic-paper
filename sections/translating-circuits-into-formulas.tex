...
% We show in the next subsection how to useIn the next subsection we use this
% succinct encoding, along with the fixed-point operator, to evaluate the entire
% circuit.
 
% We first show that the evaluation of $g$ depends only what $\gamma$ maps to
% $\consp(g)$, and as such we only need to characterise those assignments to the
% support of $g$ for which $g$ evaluates to true. In the next subsection we show
% that this recursive step may be defined in $\FPR$, and hence that the circuit
% (and family of circuits) can be evaluated in $\FPR$.
