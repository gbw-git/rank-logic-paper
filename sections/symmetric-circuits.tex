...
We abbreviate $(\vec{a}, R) \in \ind(\tau, D)$ by $\vec{a}_R$, and refer
to $R$ as \emph{the type} of $\vec{a}_R$. We often abuse notation and treat
$\vec{a}_R$ as the tuple $\vec{a}$ in $R^D$. In the event that there is only one
relational symbol in $\tau$ we identify $\vec{a}_R$ with $\vec{a}$. 
...
many-sorted set $D := \biguplus_{s \in S} D_{s}$, let $X := \ind (\tau, D)$ and $G \leq \sym_{X}$.
There is an obvious action of $G$ on $X$. There is also an action of the
automorphism group $\str{\tau, D}$ on $X$ defined for the automorphism $\sigma$
by $\sigma \cdot \vec{a}_R := (\sigma \vec{a})_R$. We denote the group of
automorphisms of $\str{\tau, D}$ by $\aut(\tau, D)$, and identify it with the
subgroup of $\sym_X$ that acts equivalently on $X$.
...
isomorphism $\pi :\str{\tau, D} \rightarrow \str{\tau, D'}$ such that $f (\vec{a}_R) =
g (\pi \vec{a}_R)$ for all $\vec{a}_R \in \ind(\tau, D)$. It is easy to see that
...
$g$ are isomorphism-equivalent if, and only if, they are $\aut(\tau,
D)$-equivalent.

In this paper we will often take the co-domain $H$ to be $\{0,1\}$, and so $f :
X \rightarrow \{0,1\}$. In this case we may think of $f$ as defining for each
relation $R^D_i$ a subset of that relation, and so determining a
$\tau$-structure over the universe $D$. We note that two functions $f, g: X
\rightarrow \{0,1\}$ are isomorphism-equivalent if, and only if, the two
$\tau$-structures they determine are isomorphic.
...
single-sorted and consists of a single unary relation symbol. Then any structured
...
reproduced using structured functions. This is because structured functions
index the input string using the elements of the relations of the complete
$\tau$-structure (in this case, edges in the complete graph). As such, the input
string is indexed by ordered pairs of elements, rather than two-element
subsets, and as such the symmetry condition on the relation is not enforced. As
such, in the structured function case the function decides properties of
directed graphs, rather than ordinary graphs. We should note, however, that the symmetry
of the edge relation is a property of directed-graph, and as such can be decided
by an isomorphism-invariant structured function over the graph vocabulary.

% , and any
% input sequence encodes a sub-relation of that complete relation. As such, the
% function  This is because in the event that $\tau := {E}$ (where $E$ is
% a binary relation, thought of as the edge relation) there is no assurance that
% the function $f : \ind
% (\tau, D) \rightarrow \{0,1\}$ is encoding

% as the structured functions are rather defined over directed
% graphs.
...
\nats{N}$ such that $D_{s} = [d_s]$. We also assume that every $(\tau,
...
takes as input structures of size $n$. We often consider families of circuits of
the form $(C_n)_{n \in \nats}$, and here we assume that each $C_n$ is a circuit
of order $n$. For a gate $g \in C$, we define the \emph{index} of $g$, denoted
by $\ind(g)$, as $\ind(\Sigma(g))$. We define the relation $W \subseteq G^2$ by
...
the universe by $\universe{g}$. We also let $\aut(g) := \aut(\vocab{g}, \universe{g})$ and
$\str{g} := \str{\vocab{g}, \universe{g}}$.
...
\item if $g$ is an internal gate such that $\Sigma (g)$ let $L^{\gamma}_g:
  ...
  if, and only if, $\Sigma(g) (L^{\gamma}_g) = 1$.
\end{enumerate}
...
\begin{proof}
  ...
  having it's input string indexed by the elements of $\ind(\rho, [n])$, and so
  ...
  R^{\mathcal{B}}$ if, and only if, $\sigma ^{-1} f_{\mathcal{B}}(\vec{a}_R) =
  f_{\mathcal{B}}(\sigma^{-1} \vec{a}_R)= 1$.
  ...
  \begin{align*}
    C[\gamma_1 \mathcal{A}](g_{\text{o}}) = C[\gamma_2 \mathcal{A}](g_{\text{o}}) & \Leftrightarrow  F_C(f_{\gamma_1 \mathcal{A}}) = F_C (f_{\gamma_2 \mathcal{A}}) \\
                                                                                  ...
                                                                                  & \Leftrightarrow F_C(f_{\gamma_1 \mathcal{A}}) = F_C (\sigma f_{\gamma_1 \mathcal{A}}).
  \end{align*}
  ...
  isomorphism-invariance, we have $F_C(f_{\gamma_1 \mathcal{A}}) = F_C (\sigma
  f_{\gamma_1 \mathcal{A}})$, and so $C[\gamma_1 \mathcal{A}](g_{\text{o}}) =
  C[\gamma_2 \mathcal{A}](g_{\text{o}})$.
  
  Suppose $C$ is invariant. Fix the bijection $\gamma_1 \in
  [n]^{\underline{U}}$. Let $\sigma \in \sym_n$ and $f: \ind(\rho, [n])
  \rightarrow \{0,1\}$. Let $\mathcal{A} = \gamma^{-1}_1 \mathcal{B}_f$ and
  $\gamma_2 := \sigma^{-1}\gamma_1 $. Since $C$ is invariant we have $C[\gamma_1
  \mathcal{A}](g_{\text{o}}) = C[\gamma_2\mathcal{A}](g_{\text{o}})$, and so
  $F_C (\sigma f_{\gamma_1 \mathcal{A}}) = F_C(f_{\gamma_1 \mathcal{A}})$. But
  $f_{\gamma_1 \mathcal{A}} = f_{\mathcal{B}_f} = f$, and so $F_C (\sigma f) =
...
homomorphism $h$ that maps $\sym_n$ to $\aut(C)$, taking any $\sigma \in \sym_n$
...
properties are polynomial-time decidable for circuits defined over a symmetric
bases. We show in Section~\label{sec:transparency-and-unique-extension} that for
circuits that include gates computing non-symmetric functions most of these
properties are at least as hard to decide as the graph-isomorphism problem. As
such, constructing an analogous translation, as well as establishing the many
other results that rely on certain properties being polynomial-time decidable,
would be highly non-trivial.

In order to deal with this problem we limit the ways in which gates computing
non-symmetric functions can be wired into the circuit and we show in
Section~\label{sec:transparency-and-unique-extensions} that this limitation is
sufficient for our purposes.

% ourselves to circuits in which those gates that compute non-symmetric
% functions


% While we should like to establish an analogous polynomial-time computable
% translation from a general circuit to a equivalent circuit with unique
% extensions, it turns out that the inclusion of gates that compute
% non-symmetric functions greatly complicates the analysis of the circuit. We
% show in Section~\label{sec:transparency-and-unique-extensions} that most of
% the polynomial-time decidable properties used by Anderson and Dawar in order
% to define this translation are at least as hard to decide for general circuits
% as the graph-isomorphism problem.


% Since the proof of our main result requires that many circuit properties are
% polynomial-time decidable, we introduce restrictions on how gates computing
% non-symmetric functions can be wired into the circuit.



% The inclusion of gates that inclusion greatly complicates the analysis of a
% general circuit, resulting in many circuit polynomial-time decidable circuit
% properties

% It turns out that allowing non-symmetric functions greatly complicates the the
% structure of a circuit, and makes certain many circuit properties difficult to
% decide (in a technical sense).


% inclusion of non-symmetric functions in the basis makes deciding certain
% circuits properties non-trivial,


% determining certain properties

% in the basis greatly complicates the argument, unfortunately, for the general
% $(\mathbb{B}, \rho)$-circuit, establishing an exactly analogous condition is a
% non-trivial problem. We discuss this problem in more detail in
...
that if a circuit has unique labels then it has unique extensions. Before we do
so, we define the \emph{syntactic equivalence} relation on the gates of a
circuit.
...
\begin{definition}
  Let $\tau$ be a many-sorted vocabulary and $D$ and $D'$ be $\tau$-sorted sets.
  Let $X$ be a set and $\sim$ be an equivalence relation defined on $X$. Let $Y,
  Z \subset X$. We say two functions $f : \ind(\tau,D) \rightarrow X$ and $g :
  \ind(\tau, D') \rightarrow Y$ are \emph{isomorphism-equivalent up to $\sim$}
  if there exists an isomorphism $\pi : \str{\tau, D} \rightarrow \str{\tau,
    D'}$ such that $f (\vec{x}_R) \sim g (\pi \vec{x}_R)$ for all $\vec{x}_R \in
  \ind(\tau, D)$ .
\end{definition}
...
\begin{definition}
  ...
  either $g$ or $h$. Then $g \equiv h$ if, and only if, $L(g)$ and $L(h)$ are
  isomorphism equivalent up to $\equiv$.
\end{definition}
...
We could define an equivalence relation on gates that identifies a pair of gates
if, and only if, they compute the same function in the circuit. However, it is
not hard to show that deciding this equivalence relation is $\NP$-hard. The
syntactic-equivalence relation instead identifies gates that have `hereditary
similar' circuits underneath them. Since it can be shown that two
syntactically-equivalent gates must compute the same function in the circuit, it
follows that the syntactic-equivalence relation refines the above-referenced
equivalence relation.

We may also define an equivalence relation between gates identifying a pair of
gates $(h, h')$ in a circuit $C$ of order $n$ if, and only if, there exists
$\sigma \in \sym_n$, $\pi, \pi' \in \aut(C)$ both extending $\sigma$, and $g$ a
gate in $C$, such that $h = \pi (g)$ and $h' = \pi' (g)$. We now show this
relation is a refinement of syntactic-equivalence.

\begin{remark}
  It's interesting to note that if $C$ has unique extensions then this relation
  collapses to equality, syntactic-equivalence does not. Note to self: should I
  mention this? Also, the following result is equivalent to checking for just
  $\pi $
\end{remark}
...
\begin{proof}
  ...
  We now prove the result by induction on height. Suppose $g$ is a gate of
  height $0$. Then $g$ is either a relational or constant gate. In either case
  $\pi (g) = \pi' (g)$, and so $\pi(g)$ and $\pi'(g)$ are
  syntactically-equivalent.

  Suppose $g$ is an internal gate and suppose that for every gate $h$ of height
...
It is easy to show that, for a circuit $C$ of order $n$, if $\sigma \in \sym_n$
and both $\pi, \pi' \in \aut(C)$ extend $\sigma$, then for every gate $g$ in
$C$, $\pi (g)$ and $\pi'(g)$ are syntactically-equivalent.


We now introduce the \emph{unique labels} and \emph{transparency} conditions on
circuits.
...
\begin{definition}
  ...
  $g$ has injective labels and no two gates in $\img(L(g))$ are
  ...
  $\Sigma(g)$ is not a symmetric function has unique labels.
\end{definition}

There is a small technical point that needs some discussion before we continue.
The definition of a circuit allows for the inclusion of gates whose outputs need
not lead to any output gate. In other words, we can be sure, not matter the rest
of the circuit, that these gates cannot have any effect the function computed by
the circuit. We say these gates are \emph{redundant}.
...
It is easy to see that we can simply remove the redundant gates from a circuit,
and so construct from a given circuit an equivalent circuit without redundant
gates. Let $C := \langle G, \Omega, \Sigma, \Lambda, L \rangle$ be a circuit of
order $n$ and let $g$ be a redundant circuit in $C$. It is easy to see that $g$
is not the child of a non-redundant gate, nor is it a relational or output gate.
Thus we can define $C' := \langle G', \Omega, \restr{\Sigma}{G'}, \Lambda,
\restr{L}{G'} \rangle$, where $G' \subseteq G$ is the set of all non-redundant
gates in $C$.
...
Moreover, it can be shown that if $C$ is symmetric and $g'$ is in the orbit of
$g$ in $C$ then $g' \in G'$. As such, if $C$ is symmetric then so is $C'$. We
...
We note that the construction of $C'$ from $C$ in Lemma~\ref{lem:redudant-gates}
can be implemented so as to run in time polynomial in the size of $C$. As such,
for the remainder of this paper we assume every circuit has no redundant gates.
With this assumption stated we now show that every circuit with unique labels
...
\begin{prop}
  Let $C := \langle G, \Omega, \Sigma, \Lambda, L\rangle$ be a $(\mathbb{B},
  \rho)$-circuit of order $n$. If $C$ has unique labels then $C$ has unique
  extensions.
...
% \end{prop}
\begin{proof}
  Suppose $C$ has unique labels. Let $\pi_{\sigma}, \pi_{\sigma}' \in \aut(C_n)$
  ...
  syntactically-equivalent to $\pi (h)$ and, since $g$ has unique labels, $h$ is
  the only gate in $H_g$ syntactically-equivalent to $h$. It follows that $\pi
  (h) = h$.
...
\rho)$-circuit and let $g \in G$. We say a gate $g \in G$ is a \emph{symmetric
