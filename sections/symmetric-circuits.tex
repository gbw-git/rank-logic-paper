...
relation. We should note, however, that symmetry and irreflexivity are
...
\emph{parents} of $g$. We also abbreviate $W(g, \cdot)$ by $H_g$.
...
Section~\ref{sec:transparency-and-unique-extension} that for circuits that
include gates computing non-symmetric functions most of these important circuit
properties are at least as hard to decide as the graph-isomorphism problem. As
such, constructing an analogous translation, as well as establishing the
numerous other needed results whose proofs rely on certain properties being
polynomial-time decidable, would be non-trivial.
...
Sections~\ref{sec:transparency-and-unique-extensions} and~\ref{} that
transparency is sufficient to allow for the polynomial-time decidability of many
important circuit properties, while still keeping the circuit model powerful
enough for us to establish a translation from formulas to families of circuits.
...
We could think of any gate $g$ in the circuit as computing a function that maps
a given assignment to the input gates to the evaluation of the circuit at $g$
for the given assignment. While we would like to be able to identify gates that
compute the same function in this sense, it is not hard to show that deciding
this property is $\NP$-hard. The syntactic-equivalence relation is an
easier-to-compute refinement of this relation, instead identifying pairs of
gates if the circuits underneath them are hereditary similar.

We could also define a similar equivalence relation using the automorphisms of a
circuit. In this case we identify a pair of gates $(h, h')$ in a circuit $C$ of
order $n$ if, and only if, there exists $\sigma \in \sym_n$, $\pi, \pi' \in
\aut(C)$ both extending $\sigma$, and $g$ a gate in $C$, such that $h = \pi (g)$
and $h' = \pi' (g)$. We now show this relation is a refinement of
...
We now define the \emph{unique labels} and \emph{transparency} conditions on
circuits.
...
Lemma~\ref{lem:redundant-gates} can be implemented so as to run in time
polynomial in the size of $C$. As such, for the remainder of this paper we
assume, unless stated otherwise, every circuit has no redundant gates. With this
assumption in mind we now show that every circuit with unique labels has unique
extensions.
...
\end{prop}
\begin{proof}
  Suppose $C$ has unique labels. Let $\pi_{\sigma}, \pi_{\sigma}' \in \aut(C)$
...
We show in Proposition~\ref{prop:transparent-unique} that a transparent circuit
can be converted into an equivalent circuit with unique labels in time
polynomial in the size of the circuit. Since we are
working with $P$-uniform families of transparent circuits, this fact allows us
to assume, without a loss of generality, that transparent circuits have unique
labels (and so unique extensions).
