...
Sections~\ref{sec:transparency-and-unique-extensions} that transparency is
sufficient to allow for the polynomial-time decidability of many important
circuit properties, while still keeping the circuit model powerful enough for us
to establish a translation from formulas to families of circuits.
...
$\pi_\sigma \in \aut(C)$ extending $\sigma$. However, if $g$ is a gate in $C$
and $\pi_{\sigma} g = g'$ there may no `canonical' $\lambda \in \aut(g)$
witnessing the isomorphism-equivalence, i.e.\ such that $\pi L(g) = L(g')
\lambda$. It is this lack of a `canonical' choice that allows us to prove in
Section~\ref{sec:transparency-and-unique-extensions} that a great many important
circuit properties (e.g. the orbit of a gate, symmetry of a circuit, etc.) for
circuits with unique gates are at least as hard as the graph-isomorphism problem
to decide.

In contrast, it is easy to see that if $C$ has unique labels, and $g$ and $g'$
are gates in the circuit, then $L(g)$ and $L(g')$ are isomorphism-equivalent if,
and only if, $\lambda := L(g')^{-1}L(g)$ is in $\aut(g)$. We show in
Section~\ref{sec:transparency-and-unique-extensions} all of the circuit
properties of interest in this paper are polynomial-time decidable for circuits
with unique labels. Moreover, we show in
Proposition~\ref{prop:transparent-unique} that a transparent circuit can be
converted into an equivalent circuit with unique labels in time polynomial in
the size of the circuit. Since we are working with $P$-uniform families of
transparent circuits, this fact allows us to assume, without a loss of
generality, that transparent circuits have unique labels (and, from
\ref{prop:unique-children-unique-extensions} so unique extensions).
