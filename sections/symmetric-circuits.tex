...
arity. In that case $C$ computes a constant function. As such, in this paper we
always assume a circuit contains at least one relational gate with non-zero
arity. As such by assumption there exists a relational gate in $C$ such that
...
introduced by our generalisation. Anderson and Dawar's polynomial-time
translation makes indispensable of the polynomial-time decidability of many
important circuit properties for circuits defined over symmetric bases. However,
for the more general circuits discussed here, it is not known if even the most
basic circuit properties are polynomial-time decidable. Indeed, we show in
...
constructing an analogous argument, as well as establishing the numerous other
...
underneath them are `hereditary equivalent', i.e.\ the two circuits underneath
...
It is worth noting that if a circuit has injective labels and is reduced then
...
automorphism, are also uniquely defined (or, at least very least, well
...
% matrix-invariant.
We now discuss a few Boolean basis of interest in this paper and state the main
result. In most cases (e.g.\ \cite{}) a Boolean basis is taken to be a finite
sequence of symmetric Boolean functions. A binary encoding of a circuit defined
over such a basis is given by assigning to each element of the basis a unique
symbol, and then encoding the circuit in the obvious manner. Since there are
only a finite number of basis elements, it follows that if each basis element
can be computed in polynomial time then the mapping that takes a symbol $s$ and
a string $\vec{x}$ to the value of the function denoted by $s$ for input
$\vec{x}$ can be decided in polynomial-time. We call this function the
\emph{gate evaluation function}, and note that its polynomial-time decidability
is required for proving the polynomial-time evaluation of the circuit. However,
in order to allow for the inclusion of structured (possibly non-symmetric)
functions in the basis, we have instead defined a basis to be a (possibly
infinite) set of finite Boolean functions. In this case the gate evaluation
function may not even be decidable, let alone polynomial-time decidable.

\begin{definition}
  Let $\BB$ be a basis. Let $S$ be a set of symbols and let $F : \BB \rightarrow
  S$ be a bijection. We say $\BB$ is \emph{effective} if the function $E$ that
  maps a symbol $s$ and an input $x \in \dom(F(s))$ to the value
  $(F^{-1}(s))(x)$ is decidable. We say $\BB$ is \emph{$\PT$-effective} if $E$
...
Let $n \in \nats$, we have that $\AND[n] : \{0,1\}^{[n]} \rightarrow \{0,1\}$ is
the restriction (as a structured function) of the $\AND$ function to inputs of
size $n$. Similarly, we have that $\OR[n]$, $\NAND[n]$ and $\MAJ[n]$ are the
restrictions of these respective functions to strings of length $n$. We let
$\NOT: \{0,1\} \ra \{0,1\}$ be the not function, and we often identify $\NOT$
with $\NAND[1]$. In each case we use the name of the function as the symbol
denoting that function.

As such, the standard basis $\BS$ consists of the functions $\NAND[n]$,
$\AND[n]$, and $\OR[n]$ for every $n \in \nats$, and the majority basis is $\BM
:= \BS \cup \{\MAJ[n] : n \in \nats\}$. We often use $\NAND[1]$ and $\NOT$
interchangeably. We say a basis $\BB$ is \emph{symmetric} if every function in
$\BB$ is a symmetric Boolean function.
...
if, the matrix $M \in \{0,1\}^{[a] \times [b]}$ has rank at most $r$ over $\ff_p$
when the entries of $M$ are interpreted as elements of $\mathbb{F}_p$. We
identify the function $\RANK^{r}_p[a,b]$ with the symbol $\RANK^{r}_p[a,b]$.

Let $\RANK = \{\RANK^{r}_p[a,b] : a, b, r, p \in \nats, \text{ $p$ prime}\}$ and
let the \emph{rank basis} be $\RB := \BM \cup \RANK$. We say a basis $\BB$ is
\emph{matrix-symmetric} if every function in $\BB$ is symmetric or
matrix-symmetric. It is easy to see that the standard basis, the majority basis,
and the rank basis are all $\PT$-effective.
...
function. We say that $g$ is an \emph{$\AND$-gate} if $\Sigma(g) := \AND[a]$ for
some $a \in \nats$. Similarly we define an \emph{$\OR$-gates},
\emph{$\NAND$-gate} \emph{$\NOT$-gates}, \emph{$\MAJ$-gates}, and
\emph{$\RANK$-gates}.
...
\begin{prop}
  ...
  Moreover, this function is computable in time polynomial in $n$ and the
...
\begin{thm}
  ...
  family of symmetric circuits $(C_n')_{n \in \nats}$ defined over $\BM$ that
    ...
    \leq (5 \cdot \vert C_n \vert + 3) \cdot \vert C_n \vert$.
  \end{thm}
